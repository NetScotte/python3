...
class MyLogKill(logging.getLoggerClass()):
    ...
    if not LOGFILE_DIR.exists():
        LOGFILE_DIR.mkdir()
    # 日志文件路径
    LOGFILE_PATH = LOGFILE_DIR.joinpath("shepherd_killpid")
    ...
    def __init__(self, *args, **kwargs):
        super(MyLogKill, self).__init__(*args, **kwargs)
...
logkill = MyLogKill(__name__)
logkill.setLevel(level=logging.INFO)

if __name__ == "__main__":
    # 测试log用例
    logkill.debug("debug msg")
    logkill.info("info msg")
    logkill.warn("warn msg")
    logkill.error("error msg")
    logkill.critical("critial msg")
    ...
    try:
        ...
        logkill.exception('错误类型:%s, 错误信息:%s', e.__class__, e)
